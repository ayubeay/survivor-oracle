...
reset();
r = checkToolCall('place_equity_order', ORDER, mint(), SNAP, MANDATE_FOR_TESTS);
...
cases.forEach(([label, patch]) => {
  ...
  const c = checkToolCall('place_equity_order', altered, auth, SNAP, MANDATE_FOR_TESTS);
...
reset();
const once = mint();
const t1 = checkToolCall('place_equity_order', ORDER, once, SNAP, MANDATE_FOR_TESTS);
require('./execution-authorization').consume(once.authorization_id);
const t2 = checkToolCall('place_equity_order', ORDER, once, SNAP, MANDATE_FOR_TESTS);
...
r = checkToolCall('place_equity_order', ORDER, extended, SNAP, MANDATE_FOR_TESTS);
...
const short = issueAuthorization({ policyReceipt: ALLOW_RECEIPT, order: ORDER,
                                   ...
                                   mandate: MANDATE_FOR_TESTS });
r = checkToolCall('place_equity_order', ORDER, short, SNAP, MANDATE_FOR_TESTS);
...
r = checkToolCall('place_equity_order', ORDER, tampered, SNAP, MANDATE_FOR_TESTS);
t('edited authorization rejected', r.decision === 'DENY' &&
  r.authorization_failure === 'AUTHORIZATION_SIGNATURE_INVALID');

reset();
r = checkToolCall('place_equity_order', ORDER, mint(), 'snapshot_changed_since', MANDATE_FOR_TESTS);
t('snapshot drift rejected', r.decision === 'DENY' && r.authorization_failure === 'SNAPSHOT_DRIFT');

reset();
r = checkToolCall('cancel_equity_order', ORDER, mint(), SNAP, MANDATE_FOR_TESTS);
t('authorization for place does not permit cancel', r.decision === 'DENY' &&
  r.authorization_failure === 'CAPABILITY_MISMATCH');

reset();
r = checkToolCall('place_equity_order', ORDER, { authorization_id: 'fake', model_version: 'x' }, SNAP, MANDATE_FOR_TESTS);
...
(async () => {
  ...
  for (const [label, order, auth] of attempts) {
    try { await guardedCall(transport, 'place_equity_order', order, auth, SNAP, MANDATE_FOR_TESTS);
  ...
  reset();
  const good = mint();
  await guardedCall(transport, 'place_equity_order', ORDER, good, SNAP, MANDATE_FOR_TESTS);
  t('authorized action reaches transport exactly once', reached.length === 1);

  try { await guardedCall(transport, 'place_equity_order', ORDER, good, SNAP, MANDATE_FOR_TESTS);
  ...
  try { await guardedCall(failing, 'place_equity_order', ORDER, auth, SNAP, MANDATE_FOR_TESTS); } catch (e) {}
  const after = checkToolCall('place_equity_order', ORDER, auth, SNAP, MANDATE_FOR_TESTS);
  ...
  const results = await Promise.allSettled([
    guardedCall(slow, 'place_equity_order', ORDER, raced, SNAP, MANDATE_FOR_TESTS),
    guardedCall(slow, 'place_equity_order', ORDER, raced, SNAP, MANDATE_FOR_TESTS),
  ]);
  ...
  let f = checkToolCall('place_equity_order', Object.assign({}, ORDER, { notional_usd: 99999 }), forged, SNAP, MANDATE_FOR_TESTS);
  ...
  f = checkToolCall('place_equity_order', ORDER, unsigned, SNAP, MANDATE_FOR_TESTS);
  ...
  t('VERIFIED before consumption', authorizationState(sm, verifyAuthorization(
    { auth: sm, order: ORDER, capability: 'place_equity_order', currentSnapshotId: SNAP, mandate: MANDATE_FOR_TESTS })) === 'VERIFIED');
  ...
  t('CONSUMED after use', authorizationState(sm, verifyAuthorization(
    { auth: sm, order: ORDER, capability: 'place_equity_order', currentSnapshotId: SNAP, mandate: MANDATE_FOR_TESTS })) === 'CONSUMED');
  ...
  t('REJECTED is reserved for genuinely invalid', authorizationState(junk, verifyAuthorization(
    { auth: junk, order: ORDER, capability: 'place_equity_order', currentSnapshotId: SNAP, mandate: MANDATE_FOR_TESTS })) === 'REJECTED');
  const drifted = mint();
  t('INVALIDATED_BY_DRIFT on snapshot change', authorizationState(drifted, verifyAuthorization(
    { auth: drifted, order: ORDER, capability: 'place_equity_order', currentSnapshotId: 'other', mandate: MANDATE_FOR_TESTS })) === 'INVALIDATED_BY_DRIFT');
  ...
  let vr = verifyAuthorization({ auth: liveAuth, order: ORDER, capability: 'place_equity_order',
                                 currentSnapshotId: SNAP, mandate: live });
  ...
  await guardedCall(tport, 'place_equity_order', ORDER, pathAuth, SNAP, pathMandate);
  ...
  try { await guardedCall(tport, 'place_equity_order', ORDER, a3, SNAP, m2);
  ...
  try { await guardedCall(tport, 'place_equity_order', ORDER, a4, SNAP, null);
